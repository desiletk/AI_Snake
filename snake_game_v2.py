...
class Game:
    def __init__(self, board_width=20, board_height=20, use_gui=False, tick_rate=10, title='Snake'):
        ...
        if use_gui:
            pygame.font.init()
            self.screen = pygame.display.set_mode([(SEG_WIDTH+SEG_MARGIN) * self.board['width'],
                                                   (SEG_HEIGHT+SEG_MARGIN) * self.board['height']])
            pygame.display.set_caption(title)
            self.clock = pygame.time.Clock()
            self.render()

    def play(self, max_steps=50, weights=None):
        curr_step = 0
        key = 0
            ...
            if weights is not None:  # predict next move
                ...
                predicted_direction = np.argmax(np.array(forward_propagation(np.array(
                    [left_blocked, front_blocked, right_blocked,
                     ...
                     normalized_v_food[1], normalized_v_snake[1]]).reshape(-1, 7), weights))) - 1
                ...
                if self.use_gui:
                    if predicted_direction > 0:
                        self.draw_text('Predict: RIGHT', 0, 20, WHITE)
                    elif predicted_direction < 0:
                        self.draw_text('Predict: LEFT', 0, 20, WHITE)
                    else:
                        self.draw_text('Predict: STRAIGHT', 0, 20, WHITE)

                    if angle > 0:
                        if predicted_direction > 0:
                            self.draw_text('To Food: RIGHT', 0, 0, (0, 255, 0))
                            self.draw_text('Predict: RIGHT', 0, 20, (0, 255, 0))
                        else:
                            self.draw_text('To Food: RIGHT', 0, 0, WHITE)
                    elif angle < 0:
                        if predicted_direction < 0:
                            self.draw_text('To Food: LEFT', 0, 0, (0, 255, 0))
                            self.draw_text('Predict: LEFT', 0, 20, (0, 255, 0))
                        else:
                            self.draw_text('To Food: LEFT', 0, 0, WHITE)
                    else:
                        if predicted_direction == 0:
                            self.draw_text('To Food: STRAIGHT', 0, 0, (0, 255, 0))
                            self.draw_text('Predict: STRAIGHT', 0, 20, (0, 255, 0))
                        else:
                            self.draw_text('To Food: STRAIGHT', 0, 0, WHITE)

            _, _, ate = self.step(key)
            if ate:
                curr_step -= 25
            curr_step += 1
            if self.use_gui: self.draw_text('Steps left: ' + str(max_steps - curr_step), 0, 40, WHITE)

        return (self.snake.score * 0.25) + (self.snake.lifetime * 0.75)
